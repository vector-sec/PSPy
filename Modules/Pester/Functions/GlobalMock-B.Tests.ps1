...
try
{
    Describe 'Mocking Global Functions - Part Two' {
        It 'Restored the global function properly' {
            $globalFunctionExists = Test-Path Function:${directorySeparatorChar}global:$functionName
...
{
    if (Test-Path Function:${directorySeparatorChar}$functionName)
    {
        Remove-Item Function:${directorySeparatorChar}$functionName -Force
    }
}
